...
const {data: comments} = await useAsyncData<Comment []>(commentsUrl, ()=> $fetch(commentsUrl), {default: () => [] as Comment[]})
...
fetch('http://localhost:3000/api/posts/nuxt-page/comments', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({commenter: 'test', content: 'test'})
  })